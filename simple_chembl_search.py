...
async def search_similar_compounds(
    ...
    logger.debug(f"Searching ChEMBL with parameters: SMILES={smiles}, threshold={threshold}, max_results={max_results}")
    ...
    try:
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=timeout)) as session:
            ...
            async with session.get(url) as resp:
                ...
                logger.info(f"Found {len(molecules)} compounds with similarity >= {threshold}%")
                ...
                for mol in molecules[:max_results]:
                    ...
                    if not chembl_id or not canonical_smiles:
                        logger.warning(f"Skipping molecule due to missing required data: {mol.get('molecule_chembl_id', 'Unknown ID')}")
                        continue
                    ...
                    logger.debug(f"Added compound {chembl_id} with similarity {similarity}")
...
if __name__ == "__main__":
    ...
    async def test_async():
        ...
        result = await search_similar_compounds(hard_smiles, threshold=90, max_results=5)
        # add result of test_smiles
        result_test = await search_similar_compounds(test_smiles, threshold=90, max_results=5)
